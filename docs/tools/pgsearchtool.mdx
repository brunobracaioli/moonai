...
The `moonai_tools` package, which will include the PGSearchTool upon its release, can be installed using the following command:
...
<Note>
    The PGSearchTool is not yet available in the current version of the `moonai_tools` package. This installation command will be updated once the tool is released.
</Note>
